...
const EsitysKohta = ({vaihda_tyyppi,kokous_id,type,title,id,alkaa,loppuu,auki=false,avaa,save,poista,paatos,tila}) => {
 ...
 // var sisalto = (auki)?<Sisalto kokous_id={kokous_id} id={id} type={type} save={this_save}/>:"";
  ...
  if(tila!=3 && endDate!=null && endDate !=="") loppu = <Kalenteri pv={endDate.getDate()} kk = {endDate.getMonth()+1} alku={false}/>    
  let nappeja = [ "",
  ...
  return (
    <div className="esitys_item">
      ...
      {sisalto}
      {(parseInt(tila)==3 && parseInt(type)!=1)?<Paatos tila={tila}/>:""}

      
      {/*(parseInt(type)==6)?<Paatos type={6}/>:""*/}
      <div className={"nro"}/>
      {(parseInt(tila)!=3)?"":
      
      <div className={"nro_"+parseInt(tila)}/>
      }
      
      </div>
      {(parseInt(tila)!=3)?
      <div  onClick= {this_avaa} className="raahaa" id={"r"+id}></div>:""}
